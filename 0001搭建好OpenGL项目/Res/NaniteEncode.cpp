...
)
{
	...
	}
	{
		...
		std::vector<std::vector<uint32>> pageClusterDataOffsets;
		std::vector<std::vector<uint32>> pageClusterIndexDataOffsets;
		uint32 groupPartCount=GroupParts.Num();
		UE_LOG( LogStaticMesh, Log, TEXT("Export Nanite Mesh Start"));
//page count
//page 0 offset
//...
//page n offset
...
//cluster n data indexes 0~n
		uint32 clusterDataOffset=0;
		std::vector<uint32> clusterDataOffsets;
		std::vector<uint32> clusterIndexDataOffsets;
		uint32 currentPageIndex=0xFFFFFFFFu;
		uint32 pageDataOffset=sizeof(uint32)+Pages.Num()*sizeof(uint32);
		for (uint32 groupPartIndex=0;groupPartIndex<groupPartCount;groupPartIndex++)
		{
			FClusterGroupPart&clusterGroupPart=GroupParts[groupPartIndex];
			if(currentPageIndex!=clusterGroupPart.PageIndex)
			{
				if(currentPageIndex!=0xFFFFFFFFu)
				{
					pageClusterDataOffsets.push_back(clusterDataOffsets);
					clusterDataOffsets.clear();
					clusterDataOffset=0;
					pageClusterIndexDataOffsets.push_back(clusterIndexDataOffsets);
					clusterIndexDataOffsets.clear();
				}
				currentPageIndex=clusterGroupPart.PageIndex;
				pageDataOffsets.push_back(pageDataOffset);
				pageDataOffset+=sizeof(uint32);//cluster count
			}
			uint32 clusterNum=clusterGroupPart.Clusters.Num();
			clonedBVHData[clusterGroupPart.HierarchyNodeIndex].Misc1[clusterGroupPart.HierarchyChildIndex].ChildStartReference=clusterGroupPart.PageIndex<<8|clusterGroupPart.PageClusterOffset;
			clonedBVHData[clusterGroupPart.HierarchyNodeIndex].Misc2[clusterGroupPart.HierarchyChildIndex].ResourcePageIndex_NumPages_GroupPartSize=Groups[clusterGroupPart.GroupIndex].MipLevel << NANITE_MAX_CLUSTERS_PER_GROUP_BITS | clusterNum;
			pageDataOffset+=sizeof(uint32)*clusterNum;//cluster i data offset x cluster num
			UE_LOG( LogStaticMesh, Log, TEXT("cluster part : PageIndex(%u) PageClusterOffset(%u) Clusters.Num()(%u)"),clusterGroupPart.PageIndex,clusterGroupPart.PageClusterOffset,clusterNum);
			
			for(uint32 i=0;i<clusterNum;i++)
			{
				clusterDataOffsets.push_back(clusterDataOffset);
				uint32 clusterIndex=clusterGroupPart.Clusters[i];
				FCluster&cluster=Clusters[clusterIndex];
				uint32 indexDataOffset=sizeof(uint32)+sizeof(uint32)+sizeof(float)*4+sizeof(float)+cluster.NumVerts*sizeof(float)*3;//cluster index offset + cluster index count + 4 float + 1 float + vertex positions
				clusterDataOffset+=indexDataOffset+cluster.Indexes.Num()*sizeof(uint32);//indexDataOffset + index data
				pageDataOffset+=indexDataOffset+cluster.Indexes.Num()*sizeof(uint32);//indexDataOffset + index data
				clusterIndexDataOffsets.push_back(indexDataOffset);
				UE_LOG( LogStaticMesh, Log, TEXT("cluster %u redirect %u : cluster.NumVerts(%u) cluster.NumTris(%u) index offset(%u) cluster size(%llu)"),i,clusterIndex,cluster.NumVerts,cluster.NumTris,indexDataOffset,sizeof(uint32)+indexDataOffset+cluster.Indexes.Num()*sizeof(uint32));
			}
		}
		pageClusterDataOffsets.push_back(clusterDataOffsets);
		pageClusterIndexDataOffsets.push_back(clusterIndexDataOffsets);
		UE_LOG( LogStaticMesh, Log, TEXT("Export Nanite Mesh End"));
		{
			UE_LOG( LogStaticMesh, Log, TEXT("Export Nanite Mesh Data Start"));
			FILE*pFile=fopen(naniteMeshDataPath.c_str(),"wb");
			uint32 temp=Pages.Num();
			fwrite(&temp,sizeof(uint32),1,pFile);
			for(uint32 i=0;i<pageDataOffsets.size();i++)
			{
				temp=pageDataOffsets[i];
				fwrite(&temp,sizeof(uint32),1,pFile);
			}
			
			currentPageIndex=0xFFFFFFFFu;
			uint32 clusterPageIndex=0u;
			//export page data
			for (uint32 groupPartIndex=0;groupPartIndex<groupPartCount;groupPartIndex++)
			{
				FClusterGroupPart&clusterGroupPart=GroupParts[groupPartIndex];
				...
				if(currentPageIndex!=clusterGroupPart.PageIndex)
				{
					currentPageIndex=clusterGroupPart.PageIndex;
					clusterPageIndex=0u;
					temp=pageClusterDataOffsets[currentPageIndex].size();
					fwrite(&temp,sizeof(uint32),1,pFile);
					for(uint32 i=0;i<pageClusterDataOffsets[currentPageIndex].size();i++)
					{
						fwrite(&pageClusterDataOffsets[currentPageIndex][i],sizeof(uint32),1,pFile);
					}
				}
				for(uint32 i=0;i<clusterNum;i++)
				{
					uint32 clusterIndex=clusterGroupPart.Clusters[i];
					FCluster&cluster=Clusters[clusterIndex];
					UE_LOG( LogStaticMesh, Log, TEXT("Export Nanite Mesh Node[%u]Child[%u] Cluster MipLevel[%d] %u:%u : (%f,%f,%f,%f) %f,%f"),
						clusterGroupPart.HierarchyNodeIndex,clusterGroupPart.HierarchyChildIndex,
						cluster.MipLevel,currentPageIndex,clusterPageIndex,cluster.LODBounds.Center.X,cluster.LODBounds.Center.Y,cluster.LODBounds.Center.Z,cluster.LODBounds.W,cluster.LODError,cluster.EdgeLength);
					fwrite(&pageClusterIndexDataOffsets[currentPageIndex][clusterPageIndex++],sizeof(uint32),1,pFile);
					uint32 clusterIndexCount=cluster.Indexes.Num();
					fwrite(&clusterIndexCount,sizeof(uint32),1,pFile);
					float bounds[4]={cluster.LODBounds.Center.X,cluster.LODBounds.Center.Y,cluster.LODBounds.Center.Z,cluster.LODBounds.W};
					fwrite(bounds,sizeof(bounds),1,pFile);
					uint32 lodErrorAndEdgeLen=FFloat16(cluster.LODError).Encoded | (FFloat16(cluster.EdgeLength).Encoded << 16);
					fwrite(&lodErrorAndEdgeLen,sizeof(uint32),1,pFile);
					for(uint32 vertexIndex=0;vertexIndex<cluster.NumVerts;vertexIndex++)
					{
						FVector3f& position=cluster.GetPosition(vertexIndex);
						fwrite(&position.X,sizeof(float),1,pFile);
						fwrite(&position.Y,sizeof(float),1,pFile);
						fwrite(&position.Z,sizeof(float),1,pFile);
					}
					for(int32 indexIndex=0;indexIndex<cluster.Indexes.Num();indexIndex++)
					{
						fwrite(&cluster.Indexes[indexIndex],sizeof(uint32),1,pFile);
					}
				}
			}
			fclose(pFile);
			UE_LOG( LogStaticMesh, Log, TEXT("Export Nanite Mesh Data End"));
		}
		{
			UE_LOG( LogStaticMesh, Log, TEXT("Export Nanite Mesh BVH Start"));
			uint32 hierarchyNodesNum=clonedBVHData.Num();
			FILE*pFile=fopen(naniteMeshBVHPath.c_str(),"wb");
			for(uint32 i=0;i<hierarchyNodesNum;i++){
				const Nanite::FPackedHierarchyNode& packedHierarchyNode=clonedBVHData[i];
				fwrite(&packedHierarchyNode,1,sizeof(packedHierarchyNode),pFile);
				UE_LOG(LogTemp, Log, TEXT("export: Nanite StaticMesh - hierarchyNode %u/%u"), i, hierarchyNodesNum);
				for(uint32 j=0;j<4;j++){
					unsigned int Misc2=packedHierarchyNode.Misc2[j].ResourcePageIndex_NumPages_GroupPartSize;
					unsigned int NumChildren			= BitFieldExtractU32(Misc2, 9, 0);
					unsigned int NumPages				= BitFieldExtractU32(Misc2, 5, 9);
					unsigned int StartPageIndex			= BitFieldExtractU32(Misc2, 16, 9 + 5);
					bool isLeaf=Misc2!=0xFFFFFFFFu;
					bool isLoaded=packedHierarchyNode.Misc1[j].ChildStartReference!=0xFFFFFFFFu;
					UE_LOG(LogTemp, Log, TEXT("export: Nanite StaticMesh - %u/3 loaded[%d] misc1[%d] child reference [%u] misc2 isLeaf[%d] StartPageIndex[%u] NumPages[%u] NumChildren[%u]"), j,isLoaded, j,packedHierarchyNode.Misc1[j].ChildStartReference, isLeaf, StartPageIndex, NumPages,NumChildren);
				}
			}
			fclose(pFile);
			UE_LOG( LogStaticMesh, Log, TEXT("Export Nanite Mesh BVH End"));
		}
	}
	{
