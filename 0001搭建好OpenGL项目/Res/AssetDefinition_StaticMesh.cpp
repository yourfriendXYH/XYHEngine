...
//Engine/Plugins/Editor/EngineAssetDefinitions/Source/Private/AssetDefinition_StaticMesh.cpp
...
namespace MenuExtension_StaticMesh
{
	...
	static unsigned int BitFieldExtractU32(unsigned int input, unsigned int bitCount, unsigned int offset){
		unsigned int mask = (bitCount < 32u) ? ((1u << bitCount) - 1) : 0xFFFFFFFFu;
		return (input >> offset) & mask;
	}
	static void DumpNaniteMesh(Nanite::FResources*inNaniteMesh){
		...
		for(uint32 i=0;i<hierarchyNodesNum;i++){
			...
			UE_LOG(LogTemp, Log, TEXT("ReportMeshInfo: Nanite StaticMesh - hierarchyNode %u/%u"), i, hierarchyNodesNum);
			for(uint32 j=0;j<4;j++){
				...
				UE_LOG(LogTemp, Log, TEXT("ReportMeshInfo: Nanite StaticMesh - %u/3 loaded[%d] misc1[%d] child reference [%u] misc2 isLeaf[%d] StartPageIndex[%u] NumPages[%u] NumChildren[%u]"), j,isLoaded, j,packedHierarchyNode.Misc1[j].ChildStartReference, isLeaf, StartPageIndex, NumPages,NumChildren);
			}
		}
		fclose(pFile);
		for(uint32 i=0;i<pageStreamingStatesNum;i++){
			const Nanite::FPageStreamingState&pageStreamingState=inNaniteMesh->PageStreamingStates[i];
			if(pageStreamingState.MaxHierarchyDepth>maxBVHDepth){
				maxBVHDepth=pageStreamingState.MaxHierarchyDepth;
			}
			UE_LOG(LogTemp, Log, TEXT("ReportMeshInfo: Nanite StaticMesh - %u maxBVHDepth: %u MaxHierarchyDepth %u"), i, maxBVHDepth,pageStreamingState.MaxHierarchyDepth);
		}
	}
	static void ExecuteReportMeshInfo(const FToolMenuContext& InContext)
	{
		if (UStaticMesh* StaticMesh = UContentBrowserAssetContextMenuContext::LoadSingleSelectedAsset<UStaticMesh>(InContext))
		{
			...
			UE_LOG(LogTemp, Log, TEXT("ReportMeshInfo: Nanite StaticMesh - Nanite Mesh Enabled: [%s]"), bNaniteEnabled ? TEXT("True") : TEXT("False"));
			if(bNaniteEnabled){
				if(StaticMesh->HasValidNaniteData()){
					int32 NumVertices = StaticMesh->GetNumNaniteVertices();
					int32 NumTriangles = StaticMesh->GetNumNaniteTriangles();
					UE_LOG(LogTemp, Log, TEXT("ReportMeshInfo: Nanite StaticMesh - Nanite Mesh Valid: True"));
					UE_LOG(LogTemp, Log, TEXT("ReportMeshInfo: Nanite StaticMesh - NumVertices: %d"), NumVertices);
					UE_LOG(LogTemp, Log, TEXT("ReportMeshInfo: Nanite StaticMesh - NumTriangles: %d"), NumTriangles);
					FStaticMeshRenderData* RenderData = StaticMesh->GetRenderData();
					if (RenderData && RenderData->NaniteResourcesPtr.IsValid()) {
						DumpNaniteMesh(RenderData->NaniteResourcesPtr.Get());
					}
				}else{
					UE_LOG(LogTemp, Log, TEXT("ReportMeshInfo: Nanite StaticMesh - Nanite Mesh Valid: False"));
				}
			}
		}else{
	...
	static void GetLODMenu(UToolMenu* Menu)
	{
    	...
    	Section.AddSeparator("ReportInfo");
	    
	    {
			const TAttribute<FText> Label = LOCTEXT("StaticMesh_ReportInfo", "Report Info");
			const TAttribute<FText> ToolTip = LOCTEXT("StaticMesh_ReportInfoTooltip", "report info for the selected mesh.");
			const FSlateIcon Icon = FSlateIcon();

			FToolUIAction UIAction;
			UIAction.ExecuteAction = FToolMenuExecuteAction::CreateStatic(&ExecuteReportMeshInfo);
			UIAction.CanExecuteAction.BindLambda([](const FToolMenuContext& Context) { return UContentBrowserAssetContextMenuContext::GetNumAssetsSelected(Context) == 1; });
			Section.AddMenuEntry("StaticMesh_ReportInfo", Label, ToolTip, Icon, UIAction);
	    }
	}
