...
bool VulkanRHI::CreateRenderPass(const ST_RHIRenderPassCreateInfo* pCreateInfo, RHIRenderPass*& pRenderPass)
{
	...
	for (uint32_t i = 0; i < pCreateInfo->m_dependencyCount; ++i)
	{
		...
		vkDesc.srcStageMask = (VkPipelineStageFlags)(rhiDesc).m_srcStageMask;	// 在源子通道中，哪些管线阶段的操作需要完成
		vkDesc.dstStageMask = (VkPipelineStageFlags)(rhiDesc).m_dstStageMask;	// 在目标子通道中，哪些管线阶段可以开始执行
		vkDesc.srcAccessMask = (VkAccessFlags)(rhiDesc).m_srcAccessMask;	// 源子通道中需要完成的"内存访问操作"
		vkDesc.dstAccessMask = (VkAccessFlags)(rhiDesc).m_dstAccessMask;	// 目标子通道中需要等待的"内存访问操作"
